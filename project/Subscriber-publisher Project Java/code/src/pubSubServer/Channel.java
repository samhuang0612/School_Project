...
class Channel extends AbstractChannel {
	...
	private void notifySubscribers(AbstractEvent event) {
		...
		for(AbstractSubscriber subscriber : subscribers) {
			...
			System.out.println("Subscriber is notified and handles event Channel: Subscriber "+ subscriber.getID()+" receives event "+currentEvent.getEventType()+" and handle it at state "+subscriber.state().toString()+".");
		}
	}
